...
def main():
    ...
    if args.face.split(".")[-1] in ["jpg", "png", "jpeg", "bmp"]:
        ...
        orig_frame = cv2.resize(
            orig_frame,
            (orig_frame.shape[1] // args.resize_factor, orig_frame.shape[0] // args.resize_factor)
        )
    ...
    subprocess.run(
        ['ffmpeg', '-y', '-i', args.audio, '-ac', '1', '-strict', '-2', 'temp/temp.wav'],
    ...
    for i, (img_batch, mel_batch, frames) in enumerate(tqdm(gen, total=total_frames)):
        ...
        pred = (pred.transpose(1, 2, 0) * 255).astype(np.uint8).reshape((1, args.img_size, args.img_size, 3))
        ...
        for p, f in zip(pred, frames):
            ...
            aligned_face = (sub_face_mask * aligned_face + (1 - sub_face_mask) * aligned_face_orig).astype(np.uint8)
